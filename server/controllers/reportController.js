...
exports.outstanding = async (req, res) => {
  try {
    const bills = await Bill.findAll({
      where: {
        status: { [Op.in]: ['RECEIVED', 'APPROVED', 'DISPUTED'] },
      },
      include: [
        { model: Supplier, as: 'supplier', attributes: ['id', 'companyName', 'supplierCode', 'paymentTerms'] },
      ],
...
exports.cashflow = async (req, res) => {
  try {
    ...
    // Include overdue bills (up to 90 days past due) so attention tables show them
    ...
    const result = bills.map((bill) => ({
      ...
      billNumber: bill.billNumber,
      billDate: bill.billDate,
      supplier: bill.supplier ? bill.supplier.companyName : null,
      supplierCode: bill.supplier ? bill.supplier.supplierCode : null,
      poNumber: bill.purchaseOrder ? bill.purchaseOrder.poNumber : null,
      dueDate: bill.dueDate,
...
exports.summary = async (req, res) => {
  try {
    ...
    const [outstandingBills, dueThisWeek, paidMTD, activeSuppliers] = await Promise.all([
      Bill.findAll({
      ...
      Bill.findAll({
        where: {
          ...
          dueDate: { [Op.between]: [weekStartStr, weekEndStr] },
        },
        attributes: ['total', 'amountPaid'],
      }),
      Payment.findAll({
        where: { paymentDate: { [Op.gte]: monthStartStr } },
        attributes: ['amount'],
      }),
      Supplier.count({ where: { status: 'ACTIVE' } }),
    ]);

    const totalPayables = outstandingBills.reduce(
      (sum, b) => sum + parseFloat(b.total) - parseFloat(b.amountPaid),
      0
    );
